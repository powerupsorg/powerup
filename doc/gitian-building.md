...
*Setup instructions for a Gitian build of Powerup Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Powerup
...
- [Building Powerup Core](#building-powerup-core)
...
Clone the git repositories for powerup and Gitian.
...
in the powerup repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Powerup Core
----------------

To build Powerup Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the powerup repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/powerup/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/powerup-project/powerup
...
For example:
```bash
URL=https://github.com/thrasher-/powerup.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit powerup=${COMMIT} --url powerup=${URL} ../powerup/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit powerup=${COMMIT} --url powerup=${URL} ../powerup/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit powerup=${COMMIT} --url powerup=${URL} ../powerup/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the powerup git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../powerup/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/powerup
SIGPATH=/some/root/path/powerup-detached-sigs

./bin/gbuild --url powerup=${BTCPATH},signature=${SIGPATH} ../powerup/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/powerup-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/powerup-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/powerup-osx-build.assert
```
...
[powerup-project/gitian.sigs.ltc](https://github.com/powerup-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
