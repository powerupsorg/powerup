Powerup Core version 0.14.2 is now available from:

  <https://download.powerup.org/powerup-0.14.2.0/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://groups.google.com/forum/#!forum/powerup-dev>

Compatibility
==============

Powerup Core is extensively tested on multiple operating systems using
...
Powerup Core should also work on most other Unix-like systems but is not
...
Powerup Core now supports P2SH addresses beginning with M on mainnet and Q on testnet.
...
This only affects users that have explicitly enabled UPnP through the GUI setting or through the -upnp option, as since the last UPnP vulnerability (in Powerup Core 0.10.4) it has been disabled by default.
...
- http://testnet.poweruptools.com
- http://testnet.thrasher.io

Developers who require the new testnet blockchain paramaters can find them [here](https://github.com/powerup-project/powerup/blob/master/src/chainparams.cpp#L220).
...
Powerup Core has supported automatically pruning the blockchain since 0.13.2. Pruning
...
now ZMQ can be used on Windows. Please see [this document](https://github.com/powerup-project/powerup/blob/master/doc/zmq.md) for
...
When Powerup Core is out-of-sync on startup, a semi-transparent information
...
Commands sent over the JSON-RPC interface and through the `powerup-cli` binary
can now use named arguments. This follows the [JSON-RPC specification](http://www.jsonrpc.org/specification)
for passing parameters by-name with an object.

`powerup-cli` has been updated to support this by parsing `name=value` arguments
when the `-named` option is given.

Some examples:

    src/powerup-cli -named help command="help"
    src/powerup-cli -named getblockhash height=0
    src/powerup-cli -named getblock blockhash=000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f
    src/powerup-cli -named sendtoaddress address="(snip)" amount="1.0" subtractfeefromamount=true
...
The Alert System was disabled and deprecated in Powerup Core 0.10.4 and removed in 0.13.2. 
...
----------------------
 ...
 - A new RPC command `getmemoryinfo` has been added which will return information
   about the memory usage of Powerup Core. This was added in conjunction with
...
here, originally written for Bitcoin but can also be used for Powerup:
